    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float3 color = float3(0.06 * sin(uv.x) + redPerc, 
                    0.06 * sin(uv.y) + greenPerc, 
                    0.06 * sin(uv.x + uv.y) + bluePerc);
                float3 colorLerp = float3(0.06 * sin(uv.x) + redPercLerp, 
                    0.06* sin(uv.y) + greenPercLerp, 
                    0.06 * sin(uv.x + uv.y) + bluePercLerp);

                color = lerp(color, colorLerp, lerpValue);

				float4 colorFinal = float4(color, 1.0);
				float4 depthColor = float4(color, 0.7);
				...
				// float4 waterColor = lerp(_DepthGradientShallow, colorFinal, waterDepthDifference01);
				float4 waterColor = lerp(depthColor, colorFinal, waterDepthDifference01);
